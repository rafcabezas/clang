...
namespace {
    ...
    /// Objective-C support that are specific to the Cocotron Objective-C Runtime
...
RuntimeVersion(0), ProtocolVersion(0) {
    ...
    //?
    //CacheTy = llvm::PointerType::get(VMContext);
...
llvm::Value *CGObjCCocotron::GetClassNamed(CodeGenFunction &CGF,
                                           const std::string &Name,
                                           bool isWeak) {
...
llvm::Value *CGObjCCocotron::GetClass(CodeGenFunction &CGF,
                                      const ObjCInterfaceDecl *OID) {
    return GetClassNamed(CGF, OID->getNameAsString(), OID->isWeakImported());
}
llvm::Value *CGObjCCocotron::EmitNSAutoreleasePoolClassRef(CodeGenFunction &CGF) {
    return GetClassNamed(CGF, "NSAutoreleasePool", false);
}

llvm::Value *CGObjCCocotron::GetSelector(CodeGenFunction &CGF, Selector Sel,
                                         const std::string &TypeEncoding, bool lval) {
...
llvm::Value *CGObjCCocotron::GetSelector(CodeGenFunction &CGF,
                                         const ObjCMethodDecl *Method) {
    std::string SelTypes;
...
llvm::Constant *CGObjCCocotron::GenerateConstantString(const StringLiteral *SL) {
    ...
    if (StringClass.empty()) StringClass = "_NSConstantStringClassReference";
...
llvm::Constant *CGObjCCocotron::GenerateClassStructure(
    ...
    Elements.push_back(NULLPtr);//llvm::Constant::getNullValue(CachePtrTy)); //cache
    Elements.push_back(NULLPtr); //subclass_list
    Elements.push_back(NULLPtr); //sibling_class
    Elements.push_back(llvm::ConstantExpr::getBitCast(Protocols, PtrTy)); //protocol
    Elements.push_back(NULLPtr); //gc_object_type
    Elements.push_back(llvm::ConstantInt::get(LongTy, 1)); //abi version
    Elements.push_back(IvarOffsets); //ivar_offsets
    Elements.push_back(Properties); //properties
    Elements.push_back(StrongIvarBitmap); //strong_pointers
    Elements.push_back(WeakIvarBitmap); // weak_pointers
    // Create an instance of the structure
...
llvm::Constant *CGObjCCocotron::GenerateProtocolList(ArrayRef<std::string>Protocols){
...
llvm::Value *CGObjCCocotron::GenerateProtocolRef(CodeGenFunction &CGF,
                                                 const ObjCProtocolDecl *PD) {
...
void CGObjCCocotron::GenerateProtocolHolderCategory() {
...
llvm::Constant *CGObjCCocotron::MakeBitField(ArrayRef<bool> bits) {
...
void CGObjCCocotron::RegisterAlias(const ObjCCompatibleAliasDecl *OAD) {
...
llvm::Function *CGObjCCocotron::ModuleInitFunction() {
    ...
    if (!ClassAliases.empty()) {
        ...
        for (std::vector<ClassAliasPair>::iterator iter = ClassAliases.begin();
            ...
            TheModule.getGlobalVariable(("_OBJC_CLASS_" + iter->first).c_str(), //??__objc_exec_class
                                        true);
            if (0 != TheClass) {
...
llvm::Constant *CGObjCCocotron::GetOptimizedPropertySetFunction(bool atomic,
                                                                bool copy) {
    return 0;
}
...
llvm::Constant *CGObjCCocotron::GetCppAtomicObjectGetFunction() {
    return 0;
}
llvm::Constant *CGObjCCocotron::GetCppAtomicObjectSetFunction() {
    return 0;
}
...
llvm::Value *CGObjCCocotron::LookupIMP(CodeGenFunction &CGF,
                                       llvm::Value *&Receiver,
                                       llvm::Value *cmd,
                                       llvm::MDNode *node) {
    CGBuilderTy &Builder = CGF.Builder;
...
llvm::Value *CGObjCCocotron::LookupIMPSuper(CodeGenFunction &CGF,
                                            llvm::Value *ObjCSuper,
                                            llvm::Value *cmd) {
    CGBuilderTy &Builder = CGF.Builder;
...
clang::CodeGen::CreateCocotronObjCRuntime(CodeGenModule &CGM) {
    switch (CGM.getLangOpts().ObjCRuntime.getKind()) {
        case ObjCRuntime::Cocotron:
            return new CGObjCCocotron(CGM,1,0);
        default:
            llvm_unreachable("This is not the Cocotron runtime");
    }
